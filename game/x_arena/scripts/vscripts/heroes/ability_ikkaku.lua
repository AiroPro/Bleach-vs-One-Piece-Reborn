...
function bvo_ikkaku_skill_2_damage( keys )
	...
	local multi = ability:GetLevelSpecialValueFor("damage1", (ability:GetLevel() - 1))

	if caster:HasModifier("bvo_ikkaku_skill_3_modifier") or caster:HasModifier("bvo_ikkaku_skill_3_perma_modifier") then
		radius = ability:GetLevelSpecialValueFor("radius2", (ability:GetLevel() - 1))
		multi = ability:GetLevelSpecialValueFor("damage2", (ability:GetLevel() - 1))
	end
	...
	for _,unit in pairs(localUnits) do
		local damageTable = {
			victim = unit,
			attacker = caster,
			damage = damage + multi * caster:GetStrength(),
			damage_type = DAMAGE_TYPE_PHYSICAL,
		}
...
function bvo_ikkaku_skill_3(keys)
	...
	local sync2 = caster:FindAbilityByName("bvo_ikkaku_skill_4_off"):GetCooldownTimeRemaining()
	local sync3 = caster:FindAbilityByName("bvo_ikkaku_skill_5_off"):GetCooldownTimeRemaining()

	local level2 = caster:FindAbilityByName("bvo_ikkaku_skill_4_off"):GetLevel()
	caster:SwapAbilities("bvo_ikkaku_skill_4", "bvo_ikkaku_skill_4_off", true, true)
	caster:FindAbilityByName("bvo_ikkaku_skill_4"):SetLevel(level2)
	caster:FindAbilityByName("bvo_ikkaku_skill_4_off"):SetHidden(true)

	local level3 = caster:FindAbilityByName("bvo_ikkaku_skill_5_off"):GetLevel()
	caster:SwapAbilities("bvo_ikkaku_skill_5", "bvo_ikkaku_skill_5_off", true, true)
	caster:FindAbilityByName("bvo_ikkaku_skill_5"):SetLevel(level3)
	caster:FindAbilityByName("bvo_ikkaku_skill_5_off"):SetHidden(true)

	caster:FindAbilityByName("bvo_ikkaku_skill_4"):StartCooldown(sync2)
	caster:FindAbilityByName("bvo_ikkaku_skill_5"):StartCooldown(sync3)
...
function bvo_ikkaku_skill_3_end(keys)
	...
	local sync2 = caster:FindAbilityByName("bvo_ikkaku_skill_4"):GetCooldownTimeRemaining()
	local sync3 = caster:FindAbilityByName("bvo_ikkaku_skill_5"):GetCooldownTimeRemaining()

	local level2 = caster:FindAbilityByName("bvo_ikkaku_skill_4"):GetLevel()
	caster:SwapAbilities("bvo_ikkaku_skill_4", "bvo_ikkaku_skill_4_off", true, true)
	caster:FindAbilityByName("bvo_ikkaku_skill_4_off"):SetLevel(level2)
	caster:FindAbilityByName("bvo_ikkaku_skill_4"):SetHidden(true)

	local level3 = caster:FindAbilityByName("bvo_ikkaku_skill_5"):GetLevel()
	caster:SwapAbilities("bvo_ikkaku_skill_5", "bvo_ikkaku_skill_5_off", true, true)
	caster:FindAbilityByName("bvo_ikkaku_skill_5_off"):SetLevel(level3)
	caster:FindAbilityByName("bvo_ikkaku_skill_5"):SetHidden(true)

	caster:FindAbilityByName("bvo_ikkaku_skill_4_off"):StartCooldown(sync2)
	caster:FindAbilityByName("bvo_ikkaku_skill_5_off"):StartCooldown(sync3)
...
function bvo_ikkaku_skill_4_stack(keys)
	...
	if current_stack < 10 then caster:SetModifierStackCount("bvo_ikkaku_skill_4_modifier_boni", ability, current_stack + 1 ) end
end
...
function bvo_ikkaku_skill_5_damage(keys)
	...
	local damageTable = {
		victim = target,
		attacker = caster,
		damage = caster:GetStrength() * str_multi + (stack * 60) * charge_multi,
		damage_type = DAMAGE_TYPE_PHYSICAL,
	}
	ApplyDamage(damageTable)
	SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_POISON_DAMAGE, caster, ApplyDamage(damageTable) , nil)
