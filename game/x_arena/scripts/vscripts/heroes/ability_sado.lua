...
function bvo_sado_skill_4(keys)
	...
	local level = caster:FindAbilityByName("bvo_sado_skill_5_off"):GetLevel()
	caster:SwapAbilities("bvo_sado_skill_5", "bvo_sado_skill_5_off", true, true)
	caster:FindAbilityByName("bvo_sado_skill_5"):SetLevel(level)
	caster:FindAbilityByName("bvo_sado_skill_5_off"):SetHidden(true)
end
...
function bvo_sado_skill_4_end(keys)
	...
	local sync1 = caster:FindAbilityByName("bvo_sado_skill_5"):GetCooldownTimeRemaining()

	local level = caster:FindAbilityByName("bvo_sado_skill_5"):GetLevel()
	caster:SwapAbilities("bvo_sado_skill_5", "bvo_sado_skill_5_off", true, true)
	caster:FindAbilityByName("bvo_sado_skill_5_off"):SetLevel(level)
	caster:FindAbilityByName("bvo_sado_skill_5"):SetHidden(true)

	caster:FindAbilityByName("bvo_sado_skill_5_off"):StartCooldown(sync1)
...
function bvo_sado_skill_5_damage(keys)
	...
	local damageTable = {
		victim = target,
		attacker = caster,
		damage = ( 10 * caster:GetStrength() ) * 0.1,
		damage_type = DAMAGE_TYPE_PHYSICAL,
	}
	ApplyDamage(damageTable)
	SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_POISON_DAMAGE, caster, ApplyDamage(damageTable) , nil)
