...
function yamamoto_skill_4_dummy(keys)
	...
	local point_projectile = casterPos + forward:Normalized() * 1600
	...
	Timers:CreateTimer(1.6, function ()
		...
		if caster:IsAlive() then
			...
			ParticleManager:SetParticleControl( particle, 1, Vector(1600, 1600, 1600) )
...
function yamamoto_skill_4_burn_damage(keys)
	...
	if ability ~= nil then
		...
		local damageTable = {
			victim = target,
			attacker = caster,
			damage = caster.hero_str * multi + damage,
